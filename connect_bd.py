...
class BdConnect:
    ...
    def _add_meta_data(self, call_type, uuid):
        ...
        url = self.loginurl + '/api/sensor/' + uuid + '/metadata'
    ...
    def create_sensor_points(self):
        ...
        for key, val in self.sensor_data.iteritems():
            ...
            url = self.loginurl + '/api/sensor'
            payload = {'data' : {
                            'name': key,
                            'building': building,
                            'identifier': identifier
                        }}
            print url, self.header, payload
            iresponse = requests.post(url, headers=self.header, data=json.dumps(payload), verify=False)
            print iresponse
            iresponse = iresponse.json();
